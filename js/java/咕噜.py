...
class Spider(Spider):
    ...
    def searchContent(self, key, quick, pg='1'):
        ...
        try:
            ...
            url = f"{self.host}/index.php?s=vod-search&wd={urllib.parse.quote(key)}"
            if pg > 1:
                url += f"-p-{pg}"
            ...
            # 解析搜索结果
            root = etree.HTML(html)
            for li in root.xpath('//li[contains(@class, "list_search")]'):
                try:
                    a = li.xpath('.//a[contains(@href, "vod-read")]')
                    if not a:
                        continue
                    a = a[0]
                    href = a.get('href', '')
                    ...
                    # 标题
                    title = li.xpath('.//div[contains(@class, "vod-intro-title")]/text()')
                    vod_name = title[0].strip() if title else ''
                    if not vod_name:
                        img = li.xpath('.//img')
                        if img:
                            vod_name = img[0].get('alt', '')
                    ...
                    # 备注
                    time_elem = li.xpath('.//div[contains(@class, "vod-intro-time")]/text()')
                    vod_remarks = time_elem[0].strip() if time_elem else ''
