...
# QQ群：807916734
"""
目标站: 爱短剧 (www.clarendonmarketing.com)
SeaCMS 架构，动态分类、精准播放解析
"""
import re
import sys
import json
...
class Spider(Spider):
    def init(self, extend=""):
        self.site_url = "https://www.clarendonmarketing.com"
        self.headers = {
    ...
    def _parse_video_list(self, soup, max_count=0):
        ...
        if not items:
            ...
            for link in items:
                href = link.get('href', '')
                vod_id = re.search(r'/A13/(\d+)\.html', href)
                if not vod_id:
                    continue
        ...
        for item in items:
            link = item.select_one('a.fed-list-pics') or item.select_one('a[href*="/A13/"]')
            if not link:
                continue
            href = link.get('href', '')
            vod_id = re.search(r'/A13/(\d+)\.html', href)
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if page <= 1:
            url = f"{self.site_url}/a13/{tid}.html"
        else:
            url = f"{self.site_url}/a13/{tid}-{page}.html"
        resp = self.fetch(url, headers=self.headers)
        if not resp:
    ...
    def detailContent(self, ids):
        if not ids:
            return {"list": []}
        vod_id = ids[0]
        url = f"{self.site_url}/A13/{vod_id}.html"
        resp = self.fetch(url, headers=self.headers)
        if not resp:
            return {"list": []}
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # 方式4：直接在页面中搜索 video 标签
        video_src = re.search(r'<video[^>]+src="([^"]+)"', html)
        if video_src:
            return {"parse": 0, "url": video_src.group(1), "header": self.headers}

        # 方式5：直接搜索 m3u8/mp4 链接
